...
from syncpipe.pipeline_bridge import (
    _as_array as _bridge_as_array,
...
from syncpipe.realtest.lerique_2024 import load_lerique_dataset, _preprocess_ecg
...
def _as_array(x) -> Optional[np.ndarray]:
    """Delegate to the library extractor, which refuses to guess between a
    time axis and several signal columns.

    A local copy of this helper used to take the first numeric column, which
    silently selected ``time`` for loaders whose frame is ``time, R, theta,
    motion_intensity`` (Gordon).  Both persons share one resampled time grid,
    so the two "signals" were identical and every WCC value was 1.0 --
    mean_synchrony 1.0, switching_rate 0, dwell_time = whole trace, and
    p_gt_pseudo 0.000 against pseudo-pairs that were equally degenerate.
    Never re-inline this: the guard is the point.
    """
    return _bridge_as_array(x)
...
def load_gordon():
    ...
    out = []
    for r in recs:
        # Gordon frames carry time/R/theta/motion_intensity, so the signal
        # column must be named explicitly; _as_array refuses to guess.
        a = _as_array(r.person_a["motion_intensity"])
        b = _as_array(r.person_b["motion_intensity"])
        ...
        for dyad_dir in sorted(p for p in base.iterdir() if p.is_dir()):
            ...
            for m in ("eda", "ecg"):
                ...
                try:
                    ...
                    continue
                if m == "ecg":
                    # ECG carries its information in R-peak TIMING, not in the
                    # waveform amplitude.  Anti-aliased decimation to 2 Hz has a
                    # ~1 Hz cutoff, which removes the entire QRS complex (5-40 Hz)
                    # and leaves only baseline drift -- i.e. it measures drift
                    # synchrony, not cardiac synchrony.  Use the frozen
                    # ECG -> R-peaks -> IBI path already implemented in the library.
                    a2, a_mask = _preprocess_ecg(a, 2048.0, 2.0)
                    b2, b_mask = _preprocess_ecg(b, 2048.0, 2.0)
                ...
                if a2.size < 30 or b2.size < 30:
                    continue
                out.append(RawRecord(dyad_label=dyad, modality=m.upper(), condition=grp,
                                     person_a=a2, person_b=b2, target_hz=2.0))
    cfg = dict(hz=2.0, window=20, design_condition=None,
               l2_contrast=None,
               status="raw_adapter_heuristic",
               note="ECG: 2048 Hz -> neurokit2 R-peaks -> IBI -> 2 Hz "
                    "(_preprocess_ecg); EDA: 2048 Hz -> 2 Hz anti-aliased. "
                    "condition = group "
...
def run_dataset(name: str, records: List[RawRecord], cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if cfg.get("l2_contrast") is None:
            ...
            # 1c: group_condition_inference is ALWAYS {modality: l2_dict}
            # (one entry for single-modality data), so there is no shape to
            # sniff. Key kept as "l2_pooled" for report-format stability, but
            # the content is per-modality — never a cross-modality pool (P0-2).
            group_inf = chain.get("group_condition_inference", {})
            res["l2_pooled"] = _summarize_pm(group_inf)
            try:
