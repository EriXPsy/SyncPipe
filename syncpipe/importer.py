"""
syncpipe.importer — Multi-format data import (CSV, OpenFace, EDF, BioSignalsPlux).
...
class DataImporter:
                ...
                for device_id, device_info in meta.items():
                    if isinstance(device_info, dict):
                        ...
                        if isinstance(channel_names, list) and channel_names:
                            # Keep the real channel labels; fall back to placeholders only when the header has no usable string names.
    ...
    def merge_person_files(
        ...
        **not** fix a between-file offset.  SyncPipe is measurement
        ...
        # Effective sampling rate: explicit override > inferred from the real time vector > default_hz fallback.
        if hz is None:
