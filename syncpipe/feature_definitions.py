...
status for README/demo/manuscript Table 1 lives in ``syncpipe.feature_status``.
...
# ---------------------------------------------------------------------------
# ③ Existence gate: second-order group surrogate test
# ---------------------------------------------------------------------------
# The existence gate is NOT "any dyad significant" (a per-dyad p<0.05 is not a
# valid unit-level proposition — a p-value speaks about a population effect,
# not about an individual dyad) and NOT a dyad-majority rule. It is a
# SECOND-ORDER group test, per modality, on the frozen endpoint: the observed
# group statistic is the mean over dyads of the per-dyad observed
# peak_amplitude, and the null is the draw-wise mean of the per-dyad
# signal-level IAAFT surrogate peaks. This models dyads as a random effect
# (each dyad's autocorrelation is preserved, cross-signal coupling destroyed),
# so between-dyad heterogeneity enters the null instead of being averaged
# away. A two-tailed Phipson-Smyth p-value is computed per modality, and the
# pre-registered PRIMARY modalities are BH-corrected (m = number of primary
# modalities) so the any-of-k gate controls its family-wise error. The gate
# passes if at least one primary modality is significant after correction.
#
# The physiological rationale for the default primary set is frozen here so
# the choice is defensible a priori (NOT selected because these modalities
# "looked best" in the data — that would be circular endpoint selection):
#
...
least one primary modality is significant after BH-FDR across the primary
set (ECG and EDA are two readouts of the same autonomic-synchrony construct,
so one channel's confirmation suffices — requiring both would over-tighten
the gate)."""

EXISTENCE_GATE_ALPHA: float = 0.05
"""Significance threshold for the second-order group existence gate.

Per-modality group p-values (two-tailed Phipson-Smyth) are BH-corrected
across the primary modality set; a primary modality "supports" existence when
its corrected p is below this alpha."""
...
def compute_surrogate_threshold(
    ...
    **Per-modality pooled surrogate threshold is the canonical v1 default.**
    ...
    ``syncpipe.session_threshold.compute_session_pooled_thresholds_by_modality``,
    ...
    **Session-level / all-dyad pooling is an OPTIONAL granularity**, not the
    ...
    ``syncpipe.session_threshold``).  Condition-level thresholds (sensitivity
...
def extract_features(
    ...
    SyncPipe.  Both ``syncpipe.dynamic_features.extract_dynamic_features``
    and ``syncpipe.validation.recovery._run_single_cell`` MUST delegate
    here.
