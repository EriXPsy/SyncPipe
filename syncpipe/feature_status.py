...
FEATURE_STATUS_ROWS: List[Dict[str, object]] = [
    ...
    },
    {
        ...
        "default_audit_or_test": "second-order group surrogate test (existence); dyad-paired permutation + BH-FDR (condition contrast)",
        "recommended_use": "single pre-registered confirmatory endpoint in v1",
        "main_risk": "sensitive to window size, smoothing, and isolated high-correlation periods",
        "implemented_in_ssot": True,
    },
    {
        "feature": "peak_abs_amplitude",
        "source_level": "WCC distribution",
        "computed_from": "maximum absolute value of the smoothed WCC trace",
        "dimension": "intensity",
        "incremental_information": "strongest momentary coupling regardless of sign (anti-phase counts as magnitude)",
        "order_sensitive": False,
        "paradigm": "all",
        "status": "exploratory",
        "enters_primary_fdr": False,
        "default_audit_or_test": "signal-level IAAFT (same null as peak_amplitude); descriptive otherwise",
        "recommended_use": "Bizzego/SUSY-convergent intensity magnitude; report alongside the signed peak_amplitude so the anti-phase contribution is explicit",
        "main_risk": "conflates in-phase and anti-phase coupling (a strong negative WCC reads as 'strong synchrony' under the absolute value)",
        "implemented_in_ssot": True,
    },
    {
    ...
    },
    {
        ...
        "status": "conditional-secondary",
        "enters_primary_fdr": False,
        "default_audit_or_test": "WCC-level IAAFT null (Family L1) plus design controls; reported in parallel with the primary claim, gated by a definedness eligibility rule",
        "recommended_use": "conditional structure descriptor when thresholding is group-comparable and definedness passes the eligibility rule; report threshold metadata and definedness",
        "main_risk": "threshold dependence; undefined or unstable when few episodes exist (e.g. 76% undefined on short-trace Gordon)",
        "implemented_in_ssot": True,
    },
    {
        ...
        "enters_primary_fdr": False,
        "default_audit_or_test": "WCC-level IAAFT null (Family L1) plus design controls; reported in parallel with the primary claim, gated by a definedness eligibility rule",
        "recommended_use": "conditional structure descriptor when thresholding is group-comparable and definedness passes the eligibility rule; describe intermittent versus stable coordination patterns cautiously",
...
def _check_feature_status_consistency() -> None:
    from .feature_definitions import PRIMARY_FDR_FAMILY, FEATURE_TIER
    ...
    for _row in FEATURE_STATUS_ROWS:
        ...
        _ssot_fdr = _name in PRIMARY_FDR_FAMILY
        if _enters_fdr != _ssot_fdr:
            raise AssertionError(
                f"feature_status row '{_name}' has enters_primary_fdr={_enters_fdr} "
                f"but SSoT membership (name in PRIMARY_FDR_FAMILY) is {_ssot_fdr}. "
                f"`enters_primary_fdr` marks the single pre-registered primary "
                f"endpoint (peak_amplitude); conditional-secondary / exploratory "
                f"features must set it False. Update the status table to match "
                f"the SSoT."
            )
...
def feature_status_latex(
    ...
    *,
    ...
    label: str = "tab:syncpipe_feature_status",
) -> str:
