...
Trace construction — a transparent default synchrony trace based on Windowed Cross-Correlation (WCC), with clear limitations.
Feature descriptor table — a structured set of WCC-derived descriptors with explicit source, incremental information, paradigm restrictions, and risk notes.
Standardized procedure — a reproducible sequence from quality control to feature extraction to inference.
Null-model audits — signal-level and design-level tests that clarify what a positive synchrony result does and does not rule out.
Governance — a single source of feature math, method logs, tests, and exported artifacts so that definitions do not silently drift across papers.
...
Current status: pre-publication validation; methods-paper development; no production user base yet.
---
...
Mathematical SSoT: `multisync/feature_definitions.py` contains the implementation-level definitions of WCC-derived features. Other modules should import feature math from here rather than reimplementing it.
Communication SSoT: `multisync/feature_status.py` contains the external-facing v1 feature status table used for README, demo exports, and manuscript Table 1 drafts.
