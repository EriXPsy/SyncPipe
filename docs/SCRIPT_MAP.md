...
| `experimental/scripts/run_lerique_shuffle.py` | [AUC] | **E** | AUC 加入顺序鲁棒性。dataset-local robustness check，已移至 experimental/，不依赖外部复现。 |
| `surrogate_controls.py` | [REAL][AUDIT] | **? → 偏 K** | Lerique post-hoc specificity 检验（排除 nuisance 假设）。**建议保留**。请确认。 |
| `analyze_all_gt.py` | [GT] | **E → 已归档** | 综合 GT 分析（PGT-2+PGT-3+EGT-4），已被单独 run_* 取代；移至 `archive/scripts/`。 |
...
2. `analyze_all_gt.py` 已归档到 `archive/scripts/`（被单独的 `run_pgt2/3_grid.py`+`run_egt4_matrix.py` 取代）。
